...
const almacen = require("../models/movimientosAlmacenPL");
...
router.get("/listarFiltroFecha", async (req, res) => {
    const { idArticuloAPL, inicio, fin } = req.query;
    almacen
        .find({ idArticuloAPL, createdAt: { $gte: new Date(inicio+'T00:00:00.000Z'), $lte: new Date(fin+'T23:59:59.999Z') } })
        .sort({ _id: -1 })
        .then((data) => res.json(data))
        .catch((error) => res.json({ message: error }));
});
